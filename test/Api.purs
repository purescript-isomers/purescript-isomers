...
-- | XXX: Commented out after accum intro
-- import Prelude
-- 
...
-- import Isomers.Request.Accum.Record (Root, Root', empty) as Request.Accum.Record
...
-- requestDuplex ::
--   ∀ reqBody res.
--   Request.Accum.Record.Root' reqBody ( productId ∷ Int )
-- requestDuplex = Request.Accum.insert (SProxy ∷ SProxy "productId") (Request.Duplex.int Request.Duplex.segment)
-- 
-- newtype HTML
--   = HTML String
-- 
...
-- admin =
--   Spec.Builder.spec
--     { dashboard: Request.Accum.Record.empty /\ (responseDual Json.Duals.int) : HNil
--     , profile:
--         { edit: Method
...
--         }
--     , products:
--       { "": get $ Request.Accum.Record.empty /\ (responseDual  (Json.Duals.arrayOf Json.Duals.int) : HNil)
--       , test3: Method
--           { "GET": requestDuplex /\ (responseDuplex : HNil)
--           , "POST": requestDuplex /\ (responseDuplex : HNil)
--           }
