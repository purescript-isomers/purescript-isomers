...
import Isomers.HTTP.ExchangeF (request, response, toExchange) as ExchangeF
...
newtype BuilderBase resp dual a b doc = BuilderBase
  { dual :: dual a -> dual b
  , extract :: b -> a
  , render :: resp b -> doc
  }
...
newtype Builder router req res err a b doc = Builder
  ( BuilderBase
...
builder
  :: forall a doc err req res router st
...
builder dual (Renderer constructor) =
  let
    dual'
      :: Json.Tokenized.Duals.Pure (FieldMissing + err) a -> Json.Tokenized.Duals.Pure (FieldMissing + err) (st /\ a)
    dual' a =
