...
instance mappingRenderLeaf ::
  ...
  mappingWithIndex (RenderStep client) prop (Tagged (Renderer renderer)) creq clientRouter = do
    ...
    res <- fetch creq
    pure $ renderer
      (Type.Equality.from clientRouter /\ HTTP.Exchange (Type.Equality.to $ creq) (Just $ map Type.Equality.to res))
...
instance instanceFoldRender ::
  ...
  , HFoldlWithIndex (Client.RequestBuildersStep (Variant ireq) (Variant ireq)) {} (Proxy (Variant ireq))
      { | requestBuilders }
...
instance foldingContractRequest ::
  ...
  FoldingWithIndex (ContractRequest vReq)
    (Proxy sym)
    (Variant vReq -> Maybe (Variant rndReq))
    (Tagged ct rnd)
    (Variant vReq -> Maybe (Variant rndReq')) where
...
instance foldingContractRequestRec ::
  ...
  , HFoldlWithIndex (ContractRequest specSubReq) (Variant specSubReq -> Maybe (Variant ())) { | renderers }
      (Variant specSubReq -> Maybe renderers')
  , Row.Cons sym renderers' rndReq rndReq'
  , Row.Union rndReq rndReq_ rndReq'
  , Row.Union specReq_ specReq__ specReq
  ) =>
  FoldingWithIndex (ContractRequest specReq)
    (Proxy sym)
    (Variant specReq -> Maybe (Variant rndReq))
    { | renderers }
    (Variant specReq -> Maybe (Variant rndReq')) where
...
contractRequest
  :: forall oreq renderers rndReq
   . HFoldlWithIndex (ContractRequest oreq) (Variant oreq -> Maybe (Variant ())) renderers
       (Variant oreq -> Maybe (Variant rndReq))
  => Proxy (Variant oreq)
  -> renderers
  -> (Variant oreq -> Maybe (Variant rndReq))
contractRequest _ renderers = hfoldlWithIndex (ContractRequest :: ContractRequest oreq)
...
instance foldingExpandRequest ::
  ...
  FoldingWithIndex (ExpandRequest specReq)
    (Proxy sym)
...
instance foldingExpandRequestRec ::
  ( IsSymbol sym
  , HFoldlWithIndex (ExpandRequest specSubReq) (Variant () -> Variant specSubReq) { | renderers }
      (Variant rndSubReq -> Variant specSubReq)
  , Row.Cons sym (Variant specSubReq) specReq_ specReq
  ...
  foldingWithIndex _ prop expandRndReq rs = do
    let
      ...
      expandRndSubReq = hfoldlWithIndex (ExpandRequest :: ExpandRequest specSubReq)
        (Variant.case_ :: Variant () -> Variant specSubReq)
        rs
    Variant.on
      prop
