...
data Insert (l :: Symbol) a = Insert (Proxy l) a
...
newtype Delete (l :: Symbol) = Delete (Proxy l)
...
newtype Get (l :: Symbol) = Get (Proxy l)
...
instance foldingGetUnit :: (IsSymbol l, Row.Cons l a r_ r) => Folding (Get l) Unit { | r } a where
  folding _ _ r = Record.get (Proxy :: Proxy l) r
else instance foldingFst :: (IsSymbol l, Row.Cons l a r_ r) => Folding (Get l) acc { | r } (a /\ acc) where
  folding _ acc r = Record.get (Proxy :: Proxy l) r /\ acc
