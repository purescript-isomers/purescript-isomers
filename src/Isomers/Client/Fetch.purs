...
fromFetchResponse res = do
  ...
  let
    headers =
      Map.fromFoldable <<< map (lmap CaseInsensitiveString) <<< Web.Fetch.Headers.toArray <<< Web.Fetch.Response.headers
        $ res
    ...
    toFiber :: forall a. Effect (Web.Promise a) -> Effect (Fiber a)
