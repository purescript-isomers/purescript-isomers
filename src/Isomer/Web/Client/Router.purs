module Isomer.Web.Client.Router where
...
-- import Isomer.Api.Client.Fetch (exchange) as Client.Fetch
-- import Isomer.Web.Client.Render (RenderFolding)
-- import Isomer.Web.Client.Render (render) as Client.Render
...
-- import Isomer.HTTP.Exchange (fromResponse) as Exchange
...
-- -- | a safe method.
-- print ∷ ∀ req. RequestDuplex' req → Isomer.HTTP.Exchange req String → String
-- print requestDuplex (Isomer.HTTP.Exchange request res) =
--   let
...
-- 
--   let
--     parse ∷ ∀ a. String → Either _ (Isomer.HTTP.Exchange (Variant req) a)
--     parse path =
--       Request.Duplex.parse codecs.request { method: "GET", headers: mempty, path, body: "" }
--         <#> \request →
--             Isomer.HTTP.Exchange request Nothing
-- 
-- 
...
--     Right (Isomer.HTTP.Exchange req _) → do
--       let
--         initial = Isomer.HTTP.Exchange req Nothing
-- 
--       { signal, modify } ← Signal.create initial
--       let
--         render = Client.Render.render spec
-- 
--         onRoute ∷ Isomer.HTTP.Exchange (Variant req) String → Router.Router (Isomer.HTTP.Exchange (Variant req) String) Router.Transitioning Router.Resolved Unit
--         onRoute (Isomer.HTTP.Exchange request Nothing) =
--           Router.Router do
--             p ← liftEffect $ do
...
-- 
--         onRoute (Isomer.HTTP.Exchange request (Just resp)) = continue
-- 
--         onTransition ∷ Router.Transition (Isomer.HTTP.Exchange (Variant req) String) → Effect Unit
...
--       let
--         redirect' ∷ Variant req → Effect Unit
--         redirect' route = redirect (Isomer.HTTP.Exchange route Nothing)
-- 
--         navigate' ∷ Variant req → Effect Unit
--         navigate' route = navigate (Isomer.HTTP.Exchange route Nothing)
-- 
--         submit' ∷ Variant req → Effect Unit
--         submit' route = submit (Isomer.HTTP.Exchange route Nothing)
-- 
--         print' ∷ Variant req → String
--         print' route = print codecs.request (Isomer.HTTP.Exchange route Nothing)
-- 
